...
function drawGraph(){
  ...
  const selectedPages=selectedNode?.type==='topic'?[...new Map(visibleEdges.filter(e=>e.source===selected||e.target===selected).map(e=>map.get(e.source===selected?e.target:e.source)).filter(n=>n?.type==='page').map(n=>[n.id,n])).values()].sort((a,b)=>a.label.localeCompare(b.label,'de')):[];
...
function renderGraphRelations(selectedNode,selectedPages){const box=$('#graphRelations');if(!selectedNode||selectedNode.type!=='topic'){box.hidden=true;box.innerHTML='';return}box.hidden=false;box.innerHTML=`<div class="relation-head"><strong>${selectedPages.length} verbundene HTML-Seiten</strong><span>Nummern entsprechen den orangefarbenen Knoten in der Grafik.</span></div><ol>${selectedPages.map(n=>`<li><a href="${esc(n.url)}" target="_blank" rel="noopener"><strong>${esc(n.label)}</strong><small>${esc(n.url)}</small></a></li>`).join('')}</ol>`}
